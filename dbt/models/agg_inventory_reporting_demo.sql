{{
  config(
    materialized='table',
    comment='Aggregated vehicle inventory data for reporting'
  )
}}
...
with inventory_events as (
    select 
        ...
        sales_date,
        processed_at
    from {{ ref('sdp_inventory_vehicles_demo') }}
),

market_data as (
    select *
    from {{ source('master_data', 'markets') }}
),
...
vds_data as (
    select 
        ...
        model_series,
        engine_type
    from {{ source('vds_vehicle_data', 'sdp_vds_vehicle_data_demo') }}
),
