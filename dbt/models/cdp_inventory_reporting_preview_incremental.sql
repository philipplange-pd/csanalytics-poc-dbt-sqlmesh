...
with inventory_data as (
    select
        ...
        creation_date_time
    from {{ ref('sdp_inventory_vehicles_preview_incremental') }}
...
stock_days_calculated as (
    ...
    {% if is_incremental() %}
        -- Only include stock dates that are new or updated
        where stock_date >= (select max(stock_date) from {{ this }})
    {% endif %}
)
